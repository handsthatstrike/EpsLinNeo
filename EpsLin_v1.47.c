...
int IsEFE(int *in, char in_file[FILENAME_MAX])
{
  ...
  int sig_chk_tmp;				// holds one EFE byte at a time while checking signatures
  ...
  if((p=rindex(in_file,'.')) != NULL) {
	...
	if((strcasecmp(p,"efe") == 0) || (strcasecmp(p,"efa") == 0) || (strcasecmp(p,"ins") == 0) ) {
		...
		lseek(*in, (long) 0x00, SEEK_SET);
		read(*in,&sig_chk_tmp,1);
		if(sig_chk_tmp != 0x0D) return(ERR);
		lseek(*in, (long) 0x01, SEEK_SET);
		read(*in,&sig_chk_tmp,1);
		if(sig_chk_tmp != 0x0A) return(ERR);
		lseek(*in, (long) 0x2F, SEEK_SET);
		read(*in,&sig_chk_tmp,1);
		if(sig_chk_tmp != 0x0D) return(ERR);
		lseek(*in, (long) 0x30, SEEK_SET);
		read(*in,&sig_chk_tmp,1);
		if(sig_chk_tmp != 0x0A) return(ERR);
		lseek(*in, (long) 0x31, SEEK_SET);
		read(*in,&sig_chk_tmp,1);
	    ...
	    )
{
  ...
  // Convert to all capital letters for comparison purposes.
  strupr(in_file);
  ...
  if( (strcmp(in_file,"ALL")==0) || (strcmp(in_file,"*.EFE")==0) || (strcmp(in_file,"*.EFA")==0) || (strcmp(in_file,"*.INS")==0) ) {
...
int ImageCopy(char *source_file_name, char *target_file_name)
{
	...
	if((source_file_size % BLOCK_SIZE) != 0) {
	  EEXIT((stderr,"Source size has to be multiple of block size %d!\n",BLOCK_SIZE));
	  exit(ERR);
	}
